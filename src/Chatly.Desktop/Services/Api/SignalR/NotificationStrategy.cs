...
public abstract class NotificationStrategy<T>(ILogger<NotificationStrategy<T>> logger)
    : INotificationStrategy where T : NotificationMessage
{
    ...
    public async Task HandleNotificationAsync(NotificationMessage message)
    {
        ...
        {
            logger.LogWarning("Received message of type {MessageType} but expected {ExpectedType}",
                message.GetType().Name, typeof(T).Name);
            return;
        }
